...
if (isset($_REQUEST['submit']))
{
  ...
  if(mysqli_num_rows(mysqli_query($con,"select sn from jackpot_results where market='$market' AND date='$date' AND timing = '$timing'")) > 0){

    	echo "<script>window.location.href = 'jackpot-declare-result.php?error=Result%20already%20published'</script>";
    return;
    }
    
   if(strlen($open) != 2){

    $error = "Digit must be 1 digit";

        }else{
        
        $batch_id = md5($stamp.$market.rand().$open.$date.$timing.$stamp);
        
        /* INSERT RESULT */
        $sql = "INSERT INTO jackpot_results
        (market,timing,number,date,created_at)
        VALUES
        ('$market','$timing','$open','$date','$stamp')";

        // echo $sql; // shows the SQL query
        ...
        (market,result,revert,created_at,batch_id,date)
        VALUES
        ('$b_market','$batch_result','0','$stamp','$batch_id','$date')"
        );
        
        
        /* GET timing_sn FROM jackpot_markets */
        
        $timing_q = mysqli_query($con,
        "SELECT sn FROM jackpot_markets
        WHERE name='$market'
        LIMIT 1");
        ...
        $timing_row = mysqli_fetch_array($timing_q);
        $timing_sn = $timing_row['sn'];
        
        
        /* GET WINNING USERS */
        
        $get_games = mysqli_query($con,
...
// $remrk = "Jackpot for market | ".$market." Winning Number |".$open ." | for game type ".$x['game'];
        ...
        // (user,amount,type,remark,created_at,batch_id,game_id)
        // VALUES
        // ('$user','$amount','1','$remrk','$stamp','$batch_id','$sn')"
        // );
        mysqli_query($con,
        "INSERT INTO transactions
        (user,amount,wallet_before,wallet_after,type,remark,created_at,batch_id,game_id)
        VALUES
        ('$user','$amount','$wallet_before','$wallet_after','1','$remrk','$stamp','$batch_id','$sn')"
        );
        ...
        $body = $market.' '.$timing.' Jackpot Result';
        
        sendNotification($body,$result,"result");
        
        } 
    if(isset($_REQUEST['redirect_back'])){
        header("location:jackpot-declare-result.php");
    }
}
...
<section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Jackpot Declare Result</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
              <li class="breadcrumb-item active">Jackpot Declare Result</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                           ...
                           <select id="gameId" name="gameId" class="form-control select2bs4" style="width: 100%;">
                                ...
                                <?php 
                                $gameList = mysqli_query($con, "SELECT * FROM `jackpot_markets` WHERE `is_active`='1' ORDER BY sn ASC");
                            
                                while($row = mysqli_fetch_array($gameList)){
                                ?>
                                    <option value="jackpot_<?php echo $row['name']; ?>_<?php echo $row['close']; ?>">
                                        <?php echo $row['name'].' '.$row['close']; ?>
                                    </option>
                                <?php } ?>
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                <label>Digit</label>
                                <select name="open" class="form-control select2bs4" id="digit" style="width: 100%;">
                                    <option value="" selected disabled>Select Digit</option>
                ...
                <div class="table-responsive">
                  ...
                  <tbody id="tbody">
                    <?php
                        ...
                        $result = mysqli_query($con, "SELECT * FROM `jackpot_results` WHERE `date`='$currentDate' ORDER BY sn DESC");
                        $i = 1;
                    ...
                    <?php
                        ...
                        if(isset($_POST['deleteResult'])){
                            ...
                            //   mysqli_query($con," update jackpot_games set status='0', is_loss='0', win_amount = '0' where date='$resultDate' AND bazar='$mrk' AND timing_sn='$timing'");
                                ...
                                $winHistory = mysqli_query($con,  "SELECT * FROM `jackpot_markets` WHERE `market`='$gameID' AND `date`='$resultDate' AND revert='0'");
...
$('#showResult').click(function(){
    ...
    if((date) && (gameId)){
        $.ajax({    //create an ajax request to 
                type: "POST",
                url: "jackpot-show-winners-ajax.php",             
                data:{date:date, gameID:gameId,digit:digit},   //expect html to be returned                
                success: function(data){
...
$('#PrevResultFilterDate').change(function(){
    ...
    if(PrevResultdate != ''){
        $.ajax({    //create an ajax request to 
            type: "POST",
            url: "filter-jackpot-result-ajax.php",             
