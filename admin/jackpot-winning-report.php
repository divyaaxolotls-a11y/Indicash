...
// Base Query for Jackpot
$query = "SELECT jg.*, u.name as username 
          FROM jackpot_games jg
          LEFT JOIN users u ON jg.user = u.mobile
          WHERE jg.date='$current_date'";
...
if ($search_num != '') {
    $query .= " AND jg.number = '$search_num'";
}
if ($selected_type != '') {
    $query .= " AND jg.game = '".mysqli_real_escape_string($con, $selected_type)."'";
}

// ✅ Status Logic from Pill Buttons
if($status_filter == 'win') { $query .= " AND jg.status='1'"; }
elseif($status_filter == 'loss') { $query .= " AND jg.is_loss='1'"; }
elseif($status_filter == 'pending') { $query .= " AND jg.status='0' AND jg.is_loss='0'"; }
elseif($status_filter == 'cancelled') { $query .= " AND jg.status='2'"; }
...
<div class="main-wrapper">
    <h4 class="mb-3 font-weight-bold">Jackpot Bid History</h4>
