...
if (isset($_REQUEST['submit']))
{
  ...
  if(mysqli_num_rows(mysqli_query($con,"select sn from starline_results where market='$market' AND date='$date' AND timing = '$timing'")) > 0){
    ...
    // Get timing_sn from starline_timings
        $timing_q = mysqli_query($con,
            "SELECT sn FROM starline_timings
             WHERE name = '$market'
             AND close = '$timing'
             LIMIT 1"
        );
        ...
        if(mysqli_num_rows($timing_q) == 0){
            die("Timing not found");
        }
        
        $timing_row = mysqli_fetch_array($timing_q);
        $timing_sn = $timing_row['sn'];
      
        $get_games = mysqli_query($con,  "SELECT * FROM `starline_games` where bazar='$market' AND timing_sn='$timing_sn'  AND date='$date' AND status='0'  AND (number='$panna' OR number='$open')");
        // echo mysqli_num_rows($get_games);
        while($x = mysqli_fetch_array($get_games))
        {
            ...
            $rate = getRate($market,$x['game']);
            $betAmount = $x['amount'];
            $amount = $betAmount * $rate;
            // $remrk = $x['game']." ".$x['bazar']." Winning";
            $remrk = "Game:".$x['game']." | Market:".$x['bazar']." | Number:".$open." | Panna:".$panna." | Result Win";
        
            mysqli_query($con,
                "UPDATE starline_games
                 SET status='1',
                 win_amount='$amount'
                 WHERE sn='$sn'"
            );    
