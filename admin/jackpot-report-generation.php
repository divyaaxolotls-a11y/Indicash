...
$db_date = date('d/m/Y', strtotime($current_date)); // Format: 25/03/2026
...
// Join with jackpot_markets to get the closing time/label
$query = "SELECT jg.*, jm.close as timing_label 
          FROM jackpot_games jg 
          LEFT JOIN jackpot_markets jm ON jg.timing_sn = jm.sn 
          WHERE jg.date='$db_date'";

if ($selected_market != '') {
    $query .= " AND jg.bazar = '".mysqli_real_escape_string($con, $selected_market)."'";
}

if ($u_search != '') {
    $query .= " AND jg.user = '$u_search'";
}
...
if ($selected_type != '') {
    ...
    if($query_type) $query .= " AND jg.game = '$query_type'";
}

$query .= " ORDER BY jg.sn DESC";
...
// Jackpot usually uses Jodi. You can add more types if your app supports them.
$game_types_list = ['Jodi'];
$db_map = [
    'Jodi' => 'jodi'
];
?>
...
<div class="main-wrapper">
    <h4 class="text-center font-weight-bold">Jackpot Report Generation</h4>
    <form method="GET" id="filterForm">
        <div class="form-grid">
            <div>
                <label class="filter-label">Jackpot Market</label>
                <select name="market" class="app-input">
                    <option value="">All Jackpot Markets</option>
                    <?php
                    $m_q = mysqli_query($con, "SELECT name FROM jackpot_markets WHERE is_active=1 ORDER BY close ASC");
    ...
    <div class="orange-divider">
        <?php echo ($selected_market ?: 'ALL JACKPOT') . " | " . $display_date; ?>
    </div>
        ...
        <?php if($selected_type == '' || $selected_type == $type_label): ?>
            ...
            <div class="table-container" id="table-<?php echo str_replace(' ', '', $type_label); ?>">
                <div class="app-table-header">
                    ...
                    <div class="header-col">Close Time</div>
                ...
                foreach($all_bets as $bet) {
                   if(strtolower($bet['game']) == strtolower($db_map[$type_label])) {
                        ...
                        echo '<div class="col-item">'.($bet['timing_label'] ?: 'N/A').'</div>';
