...
//     SUM(CASE WHEN g.status=1 THEN (g.amount * 9.5) ELSE 0 END) AS win_total,
//     (SUM(g.amount) - SUM(CASE WHEN g.status=1 THEN (g.amount * 9.5) ELSE 0 END)) AS pl_total
// FROM users u
...
$status = $_GET['active'] ?? '1'; // default = active users

$userQuery = "SELECT 
    u.mobile, u.name, u.wallet, u.sn as user_sn,
    COALESCE(SUM(g.amount),0) AS play_total,
    COALESCE(SUM(CASE WHEN g.status=1 THEN (g.amount * 9.5) ELSE 0 END),0) AS win_total,
    (COALESCE(SUM(g.amount),0) - COALESCE(SUM(CASE WHEN g.status=1 THEN (g.amount * 9.5) ELSE 0 END),0)) AS pl_total
FROM users u
LEFT JOIN games g 
    ON u.mobile = g.user 
    AND g.date = '$date_dmY'
...
if($status == '1'){
    // users who HAVE games
    $userQuery .= " AND g.user IS NOT NULL ";
}else{
    // users who DO NOT have games
    $userQuery .= " AND g.user IS NULL ";
}
$userQuery .= "
GROUP BY u.mobile 
ORDER BY $actual_sort_column $sort 
...
if($status == '1'){
    $totalQuery = mysqli_query($con, "
        SELECT COUNT(DISTINCT u.mobile) as total
        FROM users u
        INNER JOIN games g ON u.mobile = g.user
        WHERE g.date = '$date_dmY'
    ");
    ...
    $totalQuery = mysqli_query($con, "
        ...
        FROM users u
        WHERE u.mobile NOT IN (
            SELECT user FROM games WHERE date = '$date_dmY'
        )
    ");
}
