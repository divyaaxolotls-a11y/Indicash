...
/* Update these in your existing style tag */
.trans-card {
    border-radius: 12px;
    margin-bottom: 10px;
    border: 1px solid #ddd;
...
.card-deposit { background-color: #f1fff1; border-color: #d4edda; } /* Light Green */
.card-withdraw { background-color: #fff5f5; border-color: #f8d7da; } /* Light Red */
...
.col-desc { flex: 1.5; text-align: left; }
.col-point { flex: 1; text-align: center; font-weight: bold; font-size: 16px; }
.col-bal { flex: 1; text-align: right; font-weight: 500; }
...
<?php 
if($mobile == ""){
    echo "<div class='text-center mt-4'>Select user to view transactions</div>";
}
elseif(mysqli_num_rows($result) > 0){
    while($row = mysqli_fetch_assoc($result)){
        ...
        // CSS Class and colors
        $card_class = $is_add ? 'card-deposit' : 'card-withdraw';
        $point_color = $is_add ? 'text-green' : 'text-red';
        $sign = $is_add ? '+' : '-';

        // Time Formatting
        // Note: Check if your 'created_at' is a Timestamp or a String. 
        // If it's already a date string, use strtotime().
        $timestamp = is_numeric($row['created_at']) ? $row['created_at'] : strtotime($row['created_at']);
        $display_date = date('d-m-Y', $timestamp);
        $display_time = date('h:i A', $timestamp);
?>

<div class="trans-card <?php echo $card_class; ?>">
    <div class="card-content-row">
        
        <!-- Column 1: Description -->
        <div class="col-desc">
            <div class="remark-text">
                <b><?php echo number_format($row['wallet_before'], 2); ?></b>
            </div>
            <span class="date-text">
                <?php echo $display_date; ?> <i class="far fa-clock"></i> <?php echo $display_time; ?>
            </span>
            <div class="remark-text mt-1">
                <?php echo $row['remark']; ?>
            </div>
        </div>

        <!-- Column 2: Point (The Change) -->
        <div class="col-point <?php echo $point_color; ?>">
            <?php echo $sign . number_format($row['amount'], 2); ?>
        </div>

        <!-- Column 3: Balance (Wallet After) -->
        <div class="col-bal">
            <?php echo number_format($row['wallet_after'], 2); ?>
        </div>
